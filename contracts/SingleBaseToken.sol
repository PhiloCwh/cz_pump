...
contract BuilderPumpSingleBaseToken is Ownable{
...
//global variable
    ...
    address immutable baseTokenAddr;
    ...
    constructor(address _factory,address _baseToken)Ownable(msg.sender)
    {
        factory = Ifactory(_factory);
        baseToken = IERC20(_baseToken);
        baseTokenAddr = _baseToken;
...
//业务逻辑
    ...
    function ido(address _token,uint _amount) public reEntrancyMutex() {
        require(userIdoBnbBalance[msg.sender][_token] + _amount <= userMaxIdoAmount[_token],"exceed max ido amount");
        require(tokenBnbBalance[_token] + _amount <= tokenIdoBnbAmount[_token],"exceed max ido amount");
        baseToken.transferFrom(msg.sender, address(this), _amount);
        tokenBnbBalance[_token] += _amount;
        userIdoBnbBalance[msg.sender][_token] += _amount;
    ...
    function checkUnlockRemainTime(address _token) public view returns(uint time){
        if((block.timestamp - tokenClaimTime[_token]) >= 10 * ONE_DAY){
            time = 0;
        }else{
    ...
    function renounceCreatorship(address _token) public {
        ...
        IERC20(_token).transfer(address(0),IERC20(_token).balanceOf(address(this)) - 100000000 * ONE_ETH * 10 / 100);
    ...
    function renounceCreatorshipAndSendToCZ(address _token) public {
        require(tokenCreator[_token] == msg.sender,"you are not creator");
        tokenCreator[_token] = address(0);
        IERC20(_token).transfer(0x28816c4C4792467390C90e5B426F198570E29307,IERC20(_token).balanceOf(address(this)) - 100000000 * ONE_ETH * 10 / 100);
